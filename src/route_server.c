...
int response_get_service(int client, char *torken, char *arg)
{
	...
	while(get_rest_service[i].name){
		...
		if(!strncmp(get_rest_service[i].name, torken, len) && (len == torken_len)){
			ret = get_rest_service[i].s_fuc ? 
			      get_rest_service[i].s_fuc(client, arg, arg ? strlen(arg) : 0, NULL) : 
...
int response_post_service(int client, char *torken, char *arg)
{
	...
	while(post_rest_service[i].name){
		...
		if(!strncmp(post_rest_service[i].name, torken, len) && (torken_len==len)){
			ret = post_rest_service[i].s_fuc ? 
			      post_rest_service[i].s_fuc(client, arg, strlen(arg), ptr) : 
			      response_state(client, NO_SERVICE, err_msg[NO_SERVICE]);

			return ret ? ret : response_state(client, ret, err_msg[ret]);
		}
		i++;
	}
	response_state(client, NO_SERVICE, err_msg[NO_SERVICE]);
	...
	if(strstr(ibuf, "GET")){
		...
		if((pos=strstr(ibuf, "route"))){
			...
			if((arg=strchr(torken, '\n'))){
				...
				response_state(client, FORMAT_ERR, err_msg[FORMAT_ERR]);
			}
		}
	}else{
		response_state(client, NO_SUPPORT_METHOD, err_msg[NO_SUPPORT_METHOD]);
	}
}
