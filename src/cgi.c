...
int main(int argc, char **argv)
{
	...
	if(method && !strcmp(method, "POST")){
		...
		}else if(qstr && !strncmp(qstr, GATEWAY_PREFIX, strlen(GATEWAY_PREFIX))){
		...
		pos = snprintf(outbuf+len, sizeof(outbuf)-len, "%s", ptr=ezxml_toxml(xml));
		free(ptr);
		ezxml_free(xml);
		...
		*ptr = '<';
		...
		if(!strncmp(qstr, ROUTE_PREFIX, strlen(ROUTE_PREFIX))){
			strncpy(saddr, ROUTE_SERVER_PATH, sizeof(saddr));
		}else if(!strncmp(qstr, GATEWAY_PREFIX, strlen(GATEWAY_PREFIX))){
			strncpy(saddr, GATEWAY_SERVER_PATH, sizeof(saddr));
		}else{
			fclose(fp);
			response_state(1);
